...
elec-pair 是 Emacs 的内置功能，用于自动匹配括号、引号。

## text-mode

text-mode 是 Emacs 内置的一个主要模式（major mode），用于编辑纯文本文件。它是 Emacs 默认的模式，用于处理普通的纯文本文件，如纯文本文档、笔记、README 文件等。

当你打开一个文件时，如果它没有特定的模式关联，Emacs 会自动将其设置为 text-mode。text-mode 提供了一些基本的编辑功能，如缩进、剪切、复制、粘贴等，以及基本的文本搜索和替换。

## server

Emacs 内置的 server 包提供了 Emacs 作为服务器的功能。它允许你将 Emacs 启动为一个后台进程，并通过客户端连接到该进程，实现在不同的终端或会话中共享同一个 Emacs 实例的功能。

## tab-bar
## comint

comint 是 Emacs 的内置包，用于提供与交互式进程的通信和交互功能。它为用户提供了在 Emacs 中运行和与外部程序进行交互的能力。

## imenu

imenu 是 Emacs 的内置功能，用于提供在当前缓冲区中快速导航和查找函数、变量和其他代码结构的能力。

## ibuffer

ibuffer 是 Emacs 的内置包，用于管理和操作缓冲区（Buffer）的集合。它提供了一个交互式界面，用于列出和组织当前 Emacs 会话中打开的所有缓冲区。

## xref

xref 是 Emacs 的内置包，用于提供代码跳转和引用查询的功能。它可以帮助程序员在编码过程中快速导航到函数、变量和其他代码定义的位置，并查找代码中的引用和使用情况。
