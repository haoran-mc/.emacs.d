...
;; :pre：每次执行Hydra命令之前都会执行。适用于需要在每次执行命令之前进行准备或状态更新的场景。
;; :body-pre：仅在首次激活Hydra时执行。适用于需要在Hydra激活时进行初始化操作的场景。
...
(when (bound-and-true-p hydra-examples-verbatim)
  (defhydra hydra-error (global-map "C-c m")
    "goto-error"
    ("h" first-error "first")
    ("j" next-error "next")
...
;;** Example 6: 这个例子介绍了你可以使用 C-x jjk 而不需要重复 C-x j, C-x j, C-x k
(when (bound-and-true-p hydra-examples-verbatim)
...
(defhydra hydra-buffer-menu (:color pink
                             :hint nil) ;; ← 如果你自己定义了下面的提示信息，就应该禁用默认的 hint
  "
...
(defhydra hydra-org-agenda-view (:hint none) ;; ← 如果你自己定义了下面的提示信息，就应该禁用默认的 hint
  "
