...
*download pacakge:*
# emacs 自带的 package.el 会帮助下载编译安装包。

package.el 的配置源我很不喜欢。

都用 emacs 了，使用手动安装控制力更强。

gitsubmodule 手动安装，如果有找不到的库，继续 magit-submodule 添加缺少的包。


*manage package configuration:*
使用 use-package，关闭 use-package 的 always-ensure，只使用 use-package 的配置功能。

# 是否选择摆脱 use-package？


*package package:*
增加测试分支 testing，新加的包放在测试分支里面，使用满意后合并到主分支 main。

使用 submodule 方式添加新安装包，如果还有依赖，继续 submodule。

使用不满意 submodule-remove。
